...
def drop_node(config: dict, data, shots: list, device: torch.device, results_path: str='./results/') -> None:
    '''    DropNode    '''
    ...
    def run_experiment(train_mask):
        # Instantiate the model
        model = models.GNN(in_channels, config["model"]["hidden_channels"], num_classes, 'DropNode', drop_rate=config["model"]["drop_node"]).to(device)  # DropNode
        optimizer = torch.optim.Adam(model.parameters(), **config["optimizer"])
        # Train and evaluate the model
        train_df = utils.training(model, data, train_mask, data.val_mask, optimizer, lambda_entropy=-0.5, **config["training"])
        acc, f1, _, _ = utils.test(model, data, data.test_mask)
    ...
    # Log the results to the DataFrames
    avg_acc_df.loc[len(avg_acc_df)] = ["DropNode"] + avg_accuracies
    std_acc_df.loc[len(std_acc_df)] = ["DropNode"] + std_accuracies
    avg_f1_df.loc[len(avg_f1_df)] = ["DropNode"] + avg_f1_scores
    std_f1_df.loc[len(std_f1_df)] = ["DropNode"] + std_f1_scores
...
def drop_edge(config: dict, data, shots: list, device: torch.device, results_path: str='./results/') -> None:
    '''    DropEdge    '''
    ...
    def run_experiment(train_mask):
        # Instantiate the model
        model = models.GNN(in_channels, config["model"]["hidden_channels"], num_classes, 'DropEdge', drop_rate=config["model"]["drop_edge"]).to(device)  # DropEdge
        optimizer = torch.optim.Adam(model.parameters(), **config["optimizer"])
        # Train and evaluate the model
        train_df = utils.training(model, data, train_mask, data.val_mask, optimizer, lambda_entropy=-0.5, **config["training"])
        acc, f1, _, _ = utils.test(model, data, data.test_mask)
    ...
    # Log the results to the DataFrames
    avg_acc_df.loc[len(avg_acc_df)] = ["DropEdge"] + avg_accuracies
    std_acc_df.loc[len(std_acc_df)] = ["DropEdge"] + std_accuracies
    avg_f1_df.loc[len(avg_f1_df)] = ["DropEdge"] + avg_f1_scores
    std_f1_df.loc[len(std_f1_df)] = ["DropEdge"] + std_f1_scores
...
def drop_attributes(config: dict, data, shots: list, device: torch.device, results_path: str='./results/') -> None:
    '''    DropAttributes    '''
    ...
    def run_experiment(train_mask):
        # Instantiate the model
        model = models.GNN(in_channels, config["model"]["hidden_channels"], num_classes, 'DropAttributes', drop_rate=config["model"]["drop_attr"]).to(device)  # DropAttributes
        optimizer = torch.optim.Adam(model.parameters(), **config["optimizer"])
        # Train and evaluate the model
        train_df = utils.training(model, data, train_mask, data.val_mask, optimizer, lambda_entropy=-0.5, **config["training"])
        acc, f1, _, _ = utils.test(model, data, data.test_mask)
    ...
    # Log the results to the DataFrames
    avg_acc_df.loc[len(avg_acc_df)] = ["DropAttributes"] + avg_accuracies
    std_acc_df.loc[len(std_acc_df)] = ["DropAttributes"] + std_accuracies
    avg_f1_df.loc[len(avg_f1_df)] = ["DropAttributes"] + avg_f1_scores
    std_f1_df.loc[len(std_f1_df)] = ["DropAttributes"] + std_f1_scores
